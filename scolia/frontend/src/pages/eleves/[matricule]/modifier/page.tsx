...
export default function ModifierElevePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${API}/eleves/${matricule}`, {
        method: 'POST', // Laravel accepte PUT via _method
        headers: {
      ...
      router.push(`/dashboard/eleves/${matricule}`);
    } catch (err: unknown) {
  ...
  if (fetching) return <div className="p-6 text-muted-foreground">Chargement...</div>;
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">Matricule #{matricule}</p>
      </div>
